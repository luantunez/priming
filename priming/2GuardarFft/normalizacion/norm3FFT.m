%normalizo respecto al total de las frecuencias, no por frecuencia (en cada
%banda)
        ...
        for j=1:22 %sujetos
            ...
            for k=1:30 %canales
                for l=1:size(RhoP1,3) %epocas
                    ...
                    pre=RhoP1(k,l);
                    post=Rho2(k,:,:,l);
                    RhoP2(k,:,:,l)=(post-pre)/pre;  %normalizado
                end
            end
            save(['D:\EEG\priming\fft\cond_',num2str(i),'\',frec,'\t2Norm3\s',num2str(j),'.mat'],'RhoP2','Phi2','EjeX2','EjeF2');
        end
    end
